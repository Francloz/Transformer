...
class BPE(object):
    ...
    def __init__(self, codes, merges=-1, separator='@@', vocab=None, glossaries=None):
        ...
        #     if 1910 <= n <= 1914:
        #         print("{}: {} -> {}".format(n, item, item.strip('\r\n ').split(' ')))

        # self.bpe_codes = []
        # lines = [l for l in codes]
        # for (n, item) in enumerate(codes):
        #     if n < merges or merges == -1:
        #         if n == 1911:
        #             aux = item
        #             print(self.bpe_codes[-3:])
        #         elif n == 1912:
        #             self.bpe_codes.append(tuple((aux + item).strip('\r\n ').split(' ')))
        #             print(self.bpe_codes[-4:])
        #         else:
        #             self.bpe_codes.append(tuple(item.strip('\r\n ').split(' ')))
        ...
        for i, item in enumerate(self.bpe_codes):
            if len(item) != 2:
                error = True
                sys.stderr.write(
                    'Error: invalid line {:d} in BPE codes file: {} ({})\n'.format(i + offset, ' '.join(item),
                                                                                   ' '.join(self.bpe_codes[i - 1])))
